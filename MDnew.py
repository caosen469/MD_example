...
import os
import time
os.chdir('C:\\Users\sihan\Downloads')
start = time.time()
...
@numba.njit
def cal_velocity(force,velocity):
    velocity = velocity + ((force / m) * (dt / 2 ))
    return velocity

@numba.njit(parallel=True)
def cal_position(velocity,position):
...
for t in range(0,ts):
    if t==0 :
        ...
        T[t] = get_T(K[t],N)
        P[t] = get_pressure(position,N,T[t])
        ...
        force = cal_force(position)
        velocity = cal_velocity(force,velocity) 
        position = cal_position(velocity,position) 
        force = cal_force(position)
        velocity = cal_velocity(force,velocity)         
